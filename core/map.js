...
function MapField(tiles, cols, rows) {
	...
	this.meta = createTwoDimArray(rows, cols, 0);
}
...
MapField.prototype = {
	...
	save: function() {
		data = { tiles:[], surface: [], objects: [], meta: this.meta }
	...
	getMeta: function(col, row, value) {
		return this.checkBounds(col, row) && this.meta[row][col] || 0
	},
	setMeta: function(col, row, value) {
		this.checkBounds(col, row) && (this.meta[row][col]=value) 
	}	
}
